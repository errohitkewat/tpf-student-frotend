...
import { Batch, Gender, StudentStatus } from "@/lib/type";
...
export default function Studentadd({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <Header
        title="Add Student"
        desc="Create a new student profile and assign batch."
        onBack={onBack}
      />
      ...
      >
        ...
        <Actions onBack={onBack} loading={createStudent.isPending} label="Save Student" />
      </form>
    </div>
  );
}

function Header({ title, desc, onBack }: { title: string; desc: string; onBack: () => void }) {
  return (
...
function Actions({ onBack, loading, label }: { onBack: () => void; loading: boolean; label: string }) {
  return (
    <div className="flex justify-end gap-3 border-t border-slate-100 pt-6">
      <Button type="button" variant="outline" onClick={onBack} className="rounded-xl border-slate-200">
        Cancel
      </Button>
      <Button disabled={loading} className="rounded-xl bg-slate-950 px-6 text-white hover:bg-slate-800">
